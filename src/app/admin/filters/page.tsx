...
export default async function AdminFiltersRoute() {
  const env = getPublicEnv(); const client = await createSupabaseServerClient();
  ...
  return <StoreFiltersExperience storeName="Ezzion Imports" adminEmail={session.data.email} supabaseConfig={{ supabaseUrl: env.supabaseUrl, supabaseAnonKey: env.supabaseAnonKey, adminEmail: env.adminEmail }} initialOptions={options} />;
}
