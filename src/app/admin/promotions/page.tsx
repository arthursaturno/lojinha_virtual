...
export default async function AdminPromotionsRoute() {
  ...
  if (!promotionsResult.ok || !productsResult.ok) {
    return <main className="grid min-h-screen place-items-center bg-white px-6 text-center"><p className="max-w-md text-sm font-semibold">A estrutura de promocoes ainda nao esta atualizada no Supabase. Execute as migrations de promocoes completas, incluindo marca, beneficios e RPCs.</p></main>;
  }

  if (!storeSettingsResult.ok) {
    return <main className="grid min-h-screen place-items-center bg-white px-6 text-center"><p className="text-sm font-semibold">Nao foi possivel carregar as configuracoes da loja.</p></main>;
  }

  return <PromotionsPage storeName={storeSettingsResult.data.storeName} adminEmail={sessionResult.data.email} supabaseConfig={{ supabaseUrl: env.supabaseUrl, supabaseAnonKey: env.supabaseAnonKey }} promotions={promotionsResult.data} products={productsResult.data.map((product) => ({ id: product.id, name: product.name, category: product.category, brand: product.brand }))} />;
}
