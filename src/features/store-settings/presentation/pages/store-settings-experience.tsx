...
import { createUpdateStoreSettingsUseCase } from "@/core/di/store-settings-browser";
...
export function StoreSettingsExperience({ settings, adminEmail, supabaseConfig }: StoreSettingsExperienceProps) {
  const updateStoreSettingsUseCase = useMemo(
    () => createUpdateStoreSettingsUseCase(supabaseConfig),
    [supabaseConfig],
  );
  const { state, actions } = useStoreSettingsViewModel(settings, () => updateStoreSettingsUseCase);
  ...
  return (
    <div className="min-h-screen bg-[#f7f7f5] md:grid" style={{ gridTemplateColumns: `minmax(${administrationLayout.sidebarDesktopWidth}, ${administrationLayout.sidebarDesktopWidth}) minmax(0, 1fr)` }}>
      <AdminSidebar storeName={state.settings.storeName} adminEmail={adminEmail} supabaseConfig={supabaseConfig} activeSection="settings" isMobileOpen={isMobileSidebarOpen} onOpenMobile={() => setIsMobileSidebarOpen(true)} onCloseMobile={() => setIsMobileSidebarOpen(false)} />
      ...
      {state.toast ? <AppToast tone={state.toast.tone} message={state.toast.message} onDismiss={actions.dismissToast} /> : null}
    </div>
  );
}
