...
export function PromotionEditor({ draft, products, saveStatus, onClose, onSave, onDelete, onChange, onProductRulesChange, onBenefitRuleChange, onBenefitRulesChange, onImageUpload }: PromotionEditorProps) {
  ...
  async function applyCrop() {
    ...
    try {
      ...
      const imageUrl = await onImageUpload(croppedFile);
      if (imageUrl) {
    ...
    <div className="fixed inset-0 z-40 bg-black/55" onMouseDown={onClose}>
      <aside className="absolute right-0 top-0 h-full w-full overflow-auto bg-[#f7f7f5] p-4 md:w-[620px] md:p-6" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="grid gap-4">
          ...
          {draft.kind === "popup" ? <section className="border border-[var(--color-border)] bg-white p-4"><FieldLabel>FOTOS DA CAMPANHA</FieldLabel><p className="text-[var(--color-muted)]" style={{ fontSize: administrationTypography.helper }}>Selecione ate cinco fotos. A primeira aparece primeiro no popup.</p><div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">{imageUrls.map((imageUrl, index) => <div key={imageUrl} className="relative aspect-[4/5] overflow-hidden border border-[var(--color-border)] bg-[#ededed]"><Image src={imageUrl} alt={`Foto ${index + 1} da campanha`} fill sizes="180px" className="object-cover" /><div className="absolute inset-x-0 bottom-0 flex justify-between bg-black/70 p-1"><button type="button" onClick={() => moveImage(index, -1)} disabled={index === 0} className="grid size-8 place-items-center bg-white text-black disabled:opacity-40" aria-label={`Mover foto ${index + 1} para a esquerda`}><FiChevronLeft aria-hidden="true" /></button><button type="button" onClick={() => removeImage(index)} className="grid size-8 place-items-center bg-white text-[var(--color-error)]" aria-label={`Remover foto ${index + 1}`}><FiTrash2 aria-hidden="true" /></button><button type="button" onClick={() => moveImage(index, 1)} disabled={index === imageUrls.length - 1} className="grid size-8 place-items-center bg-white text-black disabled:opacity-40" aria-label={`Mover foto ${index + 1} para a direita`}><FiChevronRight aria-hidden="true" /></button></div></div>)}</div>{imageUrls.length < 5 ? <label className="mt-3 flex min-h-12 cursor-pointer items-center justify-center gap-2 border border-dashed border-[var(--color-foreground)] px-3 font-black" style={{ fontSize: administrationTypography.action }}><FiImage aria-hidden="true" /><span>ADICIONAR FOTO ({imageUrls.length}/5)</span><input type="file" accept="image/*" className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) selectImage(file); event.currentTarget.value = ""; }} /></label> : null}</section> : null}
