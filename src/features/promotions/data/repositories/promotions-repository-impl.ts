...
export class PromotionsRepositoryImpl implements PromotionsRepository {
  ...
  async validateCoupon(input: Parameters<PromotionsRepository["validateCoupon"]>[0]) {
    try { return Result.success(await this.dataSource.validateCoupon(input)); }
    catch { return Result.failure({ type: "unknown", message: "Nao foi possivel validar o cupom." }); }
  }
}
